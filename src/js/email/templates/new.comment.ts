import type { NewCommentEmailPlaceholders } from "../../types/email/new.comment.type.js";
...
const newCommentEmail = ({
  ...
  unsubscribeUrl,
  currentYear,
}: NewCommentEmailPlaceholders): string => {
  return `<!DOCTYPE html>
...
<title>${commentAuthor} commented on ${postTitle}</title>
<!--[if mso]>
<noscript>
<xml>
      ...
      <tr>
        <td width="56" height="56" style="width:56px;height:56px;border-radius:16px;background-color:#FEF3E2;text-align:center;vertical-align:middle;" bgcolor="#FEF3E2">
          <div style="font-size:0;line-height:0;"><svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#F59E0B" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" xmlns="http://www.w3.org/2000/svg"><path d="M4 5.5h16V16H8.5L4.5 19V16H4z"/></svg></div>
            ...
            <p style="margin:0 0 14px 0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;font-size:15px;line-height:24px;color:#475569;"><strong>${commentAuthor}</strong> commented on <strong>${postTitle}</strong>:</p>
        ...
        <td style="background-color:#FEF5E7;border-left:3px solid #F59E0B;border-radius:8px;padding:14px 18px;">
          <table role="presentation" cellpadding="0" cellspacing="0">
            <tr>
              ...
              <td style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;font-size:13.5px;line-height:20px;color:#0F172A;">${commentText}</td>
            </tr>
          </table>
        </td>
      </tr>
